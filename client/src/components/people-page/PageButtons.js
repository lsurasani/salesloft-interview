...
const PageButtons = props => {
  ...
  return (
    <div style={{ marginTop: 10, textAlign: 'center' }}>
      ...
      <PrimaryButton
        onClick={() => updatePage(pages.prev_page)}
        disabled={!pages.prev_page}
      >
        {'< Previous Page'}
      </PrimaryButton>
      <p style={itemStyling}>{`Page ${pages.current_page}`}</p>
      <PrimaryButton
        onClick={() => updatePage(pages.next_page)}
        disabled={!pages.next_page}
      >
        {'Next Page >'}
      </PrimaryButton>
      <PrimaryButton
        onClick={() => updatePage(pages.total_pages)}
